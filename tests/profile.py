...
def topo_profile_along_path(lat1, lon1, lat2, lon2, depth_interpolated, depth, n_samples=1001):
    """
    ...
    depths_interpolated = np.empty_like(lats)
    for i in range(len(lats)):
        depths_interpolated[i] = depth_interpolated(lats[i], lons[i])
    ...
    return s_km, depths_interpolated, depths
...
def plot_topo_profile(s_km, depths_m, title="Profil bathymétrique le long du trajet"):
    ...
    plt.figure()
    plt.plot(s_km, -depth_m)   # signe - pour afficher la mer vers le bas
    plt.xlabel("Distance le long du trajet (km)")
    plt.ylabel("Profondeur (m, vers le bas)")
    plt.title(title)
    plt.grid(True)
...
depth_interpolated, depth = make_depth_map(lats, lons, H)

lat1, lon1 = 0.0, 150.0
lat2, lon2 = 35.0, 140.0

s_km, depths_m, depth_m = topo_profile_along_path(lat1, lon1, lat2, lon2, depth, n_samples=1201)

print(f"Distance totale: {s_km[-1]:.1f} km")
print(f"Profondeur min/max (m): {np.nanmin(depths_m):.1f} / {np.nanmax(depths_m):.1f}")

plot_topo_profile(s_km, depths_m, depth_m, title="Pacifique → Japon (profil bathy)")
